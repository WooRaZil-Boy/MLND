...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        if testing:
            ...
            # self.epsilon = math.cos(self.alpha * self.test_count)
            self.epsilon = 1.0/(self.test_count ** 2 + self.alpha * self.test_count)
            self.test_count += 1
    ...
    def get_maxQ(self, state):
        ...
        maxQ = max(self.Q[state].values()) #내부 dict values()의 최대 값
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            action = random.choice(self.valid_actions) #self.valid_actions에서 랜덤하게 하나 선택
            # action = self.valid_actions[random.randint(0, 3)]으로도 쓸 수 있다.
        else:
            if self.epsilon > random.random(): #엡실론 확률로 임의의 동작
                action = random.choice(self.valid_actions)
            else: #그렇지 않으면 가장 높은 Q값의 액션을 선택
...
def run():
    ...
    sim.run(n_test=10, tolerance=0.0000001)
